...
from utils.openmax_utils import getlabellist
...
def recalibrate_scores(weibull_model, labellist, imgarr,
    ...
    # print("imglayer: ", imglayer)
    # print("imgscore: ", imgarr['scores'])
    # print(f"[BB Debug @compute_openmax] ranked_list: {ranked_list}")
    alpha_weights = [
        ((alpharank+1) - i)/float(alpharank) for i in range(1, alpharank+1)]
    ranked_alpha = sp.zeros(10)
    for i in range(len(alpha_weights)):
        ranked_alpha[ranked_list[i]] = alpha_weights[i]
    # print("ranked_alpha:", ranked_alpha)
    ...
    for channel in range(NCHANNELS):
        ...
        for categoryid in range(NCLASSES):
            ...
            # print ('channel_distance: ',channel_distance)
            ...
            print(f"categoryid: {categoryid}, channel_scores: {channel_scores[categoryid]:.4f}, wscore:{wscore}, revise_ratio: {1 - wscore*ranked_alpha[categoryid]:.4f}, modified_wscore: {channel_scores[categoryid] * (1 - wscore*ranked_alpha[categoryid]):.4f}")
