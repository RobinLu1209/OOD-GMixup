...
def compute_mean_vector(feature):
    ...
    return torch.mean(feature, axis=0)


def compute_distances(mean_feature, feature, args):
    eu_dist, cos_dist, eucos_dist, my_dis = [], [], [], []
    for feat in feature:
        eu_dist += [spd.euclidean(mean_feature, feat)**2]
        cos_dist += [spd.cosine(mean_feature, feat)]
        eucos_dist += [spd.euclidean(mean_feature, feat)/200. + spd.cosine(mean_feature, feat)]
        my_dis += [spd.euclidean(mean_feature, feat) * args.euc + spd.cosine(mean_feature, feat) * args.cos]
        # eucos_dist += [euclidean_dist(mean_feature, feat)]
        # eucos_dist += [spd.euclidean(mean_feature, feat)/20.]
    distances = {'eucos': eucos_dist, 'cosine': cos_dist, 'euclidean': eu_dist, 'my_dis': my_dis}
    return distances
...
def create_weibull_model_prototype(model, dataloader, idx_by_class, args, device='cuda'):
    ...
    for idx, data in enumerate(dataloader):
        data = data.to(device)
        y = data.y.to(device)
        embedding, y = model.encoding(data)
        if idx == 0:
            y_all = y
    ...
    for j in num_classes:
        ...
        # print(f"BB Debug: mean {mean.shape}, embedding_i {embedding_i.shape}")
        distance = compute_distances(mean, embedding_i, args)
        feature_mean.append(mean)
        feature_distance.append(distance)
    return feature_mean, feature_distance

def create_weibull_model_prototype_ablation(model, dataloader, idx_by_class, args, device='cuda'):
    model.eval()
    for idx, data in enumerate(dataloader):
        data = data.to(device)
        y = data.y.to(device)
        embedding = model.encoding(data)
        if idx == 0:
            y_all = y
            embedding_all = embedding
        else:
...
def create_weibull_model(model, dataloader, dataset, args, device='cuda', tips='null'):
    ...
    for j in range(num_classes):
        ...
        if len(idx_i) == 0 or len(idx_i) < 50:
            return None, None
            # print(f"j = {j}, no train sample predict accurately.")
            # idx_i = random.sample(index_by_class[int(j)], 50)
        # elif len(idx_i) < 10:
            # print(f"j = {j}, accurate sample is {len(idx_i)}, too small!")
            # idx_i = random.sample(index_by_class[int(j)], 50)
        ...
        distance = compute_distances(mean, embedding, args)
        feature_mean.append(mean)
        feature_distance.append(distance)
...
def build_weibull(mean, distance, tail, distance_type='my_dis'):
    weibull_model = {}
...
def compute_weibull_score(weibull_model, mixup_embedding, mixup_y, class_num, distance_type='my_dis', print_distance=False, args=None):
    weibull_score = []

    all_mean_vec = []
    all_category_weibull = []
    for k in range(class_num):
        category_weibull = query_weibull(k, weibull_model, distance_type=distance_type)
        mean_vec = category_weibull[0]
        all_mean_vec.append(mean_vec)
        all_category_weibull.append(category_weibull)
    
    def get_k_dist(mean_vec, query_channel):
        if distance_type == 'eucos':
            # channel_distance = euclidean_dist(mean_vec, query_channel) 
            # print(f"euclidean: {spd.euclidean(mean_vec, query_channel)/200:.4f}, cos: {spd.cosine(mean_vec, query_channel):.4f}")
            channel_distance = spd.euclidean(mean_vec, query_channel)/200. + spd.cosine(mean_vec, query_channel)
            # channel_distance = spd.euclidean(mean_vec, query_channel)/20
        elif distance_type == 'euclidean':
            channel_distance = spd.euclidean(mean_vec, query_channel)**2
        elif distance_type == 'cosine':
            channel_distance = spd.cosine(mean_vec, query_channel)
        elif distance_type == 'my_dis':
            # print(f"euclidean: {spd.euclidean(mean_vec, query_channel)/20:.4f}, cos: {spd.cosine(mean_vec, query_channel):.4f}")
            channel_distance = spd.euclidean(mean_vec, query_channel) * args.euc + spd.cosine(mean_vec, query_channel) * args.cos
        else:
            print(
    ...
    for i in range(mixup_y.shape[0]):
        ...
        k = int(mixup_y[i])
        mean_vec_k = all_mean_vec[k]
        channel_distance = get_k_dist(mean_vec_k, query_channel)
        wscore = all_category_weibull[k][2][0].w_score(channel_distance)    
        weibull_score.append(wscore)

    return torch.tensor(weibull_score).cuda()


def compute_weibull_score_k(weibull_model, mixup_embedding, mixup_y, class_num, distance_type='my_dis', print_distance=False, args=None):
    weibull_score = []
    ...
    for i in range(mixup_y.shape[0]):
        ...
        for k in range(class_num):
            ...
            channel_distance = get_k_dist(mean_vec_k, query_channel)
            wscore = all_category_weibull[k][2][0].w_score(channel_distance)
            k_wscore.append(wscore)
        mixup_wscore = torch.tensor(k_wscore).cuda() * mixup_y[i]        
        weibull_score.append(mixup_wscore)

    return torch.stack(weibull_score, dim=0).cuda()
